...
commander
  ...
  .option('--bind [EXCHANGE:LIST,OF,ROUTING,KEYS]', 'Exchange and routing keys to bind to.')
...
let { exchange, routingKeys } = commander;

if (commander.bind) {
  [exchange, routingKeys] = commander.bind.split(':').map(part => part.trim());
}

options.application = {
  exchange,
  routingKeys,
  queuePrefix: commander.queuePrefix || exchange,
...
const required = ['application.exchange', 'application.routingKeys', 'connection.hostname'];
