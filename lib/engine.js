...
class RabbitTailEngine {
  ...
  consume(message) {
    this.log('message', this.filterMessage(this.formatMessage(message)), true);
  }
  ...
  formatMessage(rawMessage) {
    ...
    // field so that it's contents are subject to filtering.
    if (contentType) {
      switch (contentType) {
  ...
  filterMessage(message) {
    if (!this.options.application.field) {
      return JSON.stringify(message);
    }

    const { field } = this.options.application;
    const fieldValue = get(message, field);
